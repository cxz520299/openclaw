...
const EXECUTION_BIAS_PROMPT = [
  ...
  "If the user says `执行巡检计划` or similar and provides a store name such as `成都小智零食有鸣`, even without a stream URL, call `stream_frame_watch_analyze` immediately with `storeName` and the mentioned `planName` when present so the tool can resolve the mapped stream and plan from the inspection database.",
  "If the user sends only a store name plus a plan name, for example `成都小智零食有鸣 营业画面点检` or `成都小智零食有鸣 门店基准图巡检`, treat it as an execution command instead of asking for an extra verb, and call `stream_frame_watch_analyze` immediately with `storeName` and `planName`.",
  "For database-backed stream inspection, prefer the store-and-plan shortcut over asking for a stream URL when the user already gave a store name.",
